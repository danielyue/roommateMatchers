...
class Irving1985Mod:
    ...
    def match(self):
        ...
        if stableFound:
            ...
            return (stableFound,
                [item for sublist in self.reducedPreferences for item in sublist])
        else:
            logging.info("No Stable Match Possible")
            return (stableFound, [])
...
def main(args):
    ...
    people4 = np.array(range(4))
    example4 = np.array([[1,2,3],
                         [0,2,3],
                         [1,0,3],
                         [2,1,0]])

    matcher = Irving1985(people4, example4)
    print matcher.match()
